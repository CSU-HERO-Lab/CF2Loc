...
class DisCoLocModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            if batch_idx % viz_step == 0 and isinstance(
                ...
                for idx in range(num_viz):
                    ...
                    pred_map_viz = map_all_input[pred_idx]

                    pred_type = "unknown"
                    pose_pred_viz = None
                    if pred_idx < batch_size:
                        pred_type = "easy" if pred_idx != idx else "correct"
                    else:
                        pred_type = "hard"
                        hard_neg_idx = pred_idx - batch_size
                        pose_pred_viz = neg_pose[hard_neg_idx]
                    ...
                    img = visualize_cross_modal_batch(
                        ...
                        obs_img=obs_img[idx],
                        floorplan_img=floorplan_img[idx],
                        wh=wh[idx],
                        local_map=local_map[idx],
